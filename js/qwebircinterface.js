	...
	initialize: function(element, ui, options) {
		...
		window.addEvent("domready", function() {
			...
			if (this.options.searchURL) {
				...
				if ($defined(args["uio"])) this.options.uiOptionsArg = args["uio"];
				...
				if ($defined(url)) {
					...
					if ($defined(chans) && chans != "") canAutoConnect = true;
				} else {
					chans = args["channels"];
					...
					if (chans) {
						...
						for (var i = 0; i < chans.length; i++) {
							...
							if (chans[i].charAt(0) != '#') chans2[i] = "#" + chans2[i]
						}
						cdata[0] = chans2.join(",");
						ichans = cdata.join(" ");
						canAutoConnect = true;
				...
				if ($defined(nick)) inick = this.randSub(nick);

				if (args["randomnick"] && args["randomnick"] == 1) inick = this.options.initialNickname;
			...
			if (usingAutoNick && autoConnect) inick = this.options.initialNickname;
